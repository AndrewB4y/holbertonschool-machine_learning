...
class DeepNeuralNetwork:
    ...
    def cost(self, Y, A):
        ...
        @A: a one-hot numpy.ndarray of shape (classes, m) containing the
            activated outputs of the DeepNN for each example.

        Returns: the cost.
        """

        m = Y.shape[1]
        max_idxs = np.array([np.argmax(Y, axis=0), ])
        logs_cal = - np.log(np.take_along_axis(A, max_idxs, axis=0))
        cost = np.sum(logs_cal) / m
    ...
    def evaluate(self, X, Y):
        ...
        decode = np.amax(self.cache['A' + str(self.L)], axis=0)
        return (np.where(self.cache['A' + str(self.L)] == decode, 1, 0),
